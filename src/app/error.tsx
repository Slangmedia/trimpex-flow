...
export default function ErrorPage({ error, reset }: ErrorProps) {
  ...
  useEffect(() => {
    // Log the error to client console
    console.error("ErrorBoundary caught error:", error);

    if (error.digest) {
      setLoading(true);
      // Attempt to retrieve the unmasked server error details using the digest
      ...
      <Card className="w-full max-w-xl border-slate-800/80 bg-slate-900/60 backdrop-blur-xl text-slate-100 shadow-2xl relative z-10 overflow-hidden">
        ...
        <CardHeader className="text-center pb-4 pt-8">
          ...
          <CardDescription className="text-slate-400 text-sm mt-1 max-w-md mx-auto leading-relaxed">
            A server-side error occurred while processing this request. The system has logged this incident.
          </CardDescription>
        </CardHeader>
